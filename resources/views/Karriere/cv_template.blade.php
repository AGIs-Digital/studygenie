...
<body>
    <div class="container">
        <div class="main-content">
            <div class="header">
                <div>
                    @if(!empty($name)) <h1>{{ $name }}</h1> @endif
                </div>
            </div>
            <div class="main">
                ...
                <div class="section">
                    ...
                    <div class="entry">
                        @if(!empty($experience_position[$index])) <h3>{{ $experience_position[$index] }}</h3> @endif
                        @if(!empty($experience_period[$index])) <p>{{ $experience_period[$index] }} || {{ $company }}</p> @endif
                        @if(!empty($experience_description[$index])) <ul><li>{{ $experience_description[$index] }}</li></ul> @endif
                    </div>
                    @endforeach
                </div>
                @endif
                ...
                <div class="section">
                    ...
                    <div class="entry">
                        @if(!empty($school)) <h3>{{ $school }}</h3> @endif
                        @if(!empty($school_start[$index])) <p>{{ \Carbon\Carbon::parse($school_start[$index])->format('d.m.Y') }} - {{ \Carbon\Carbon::parse($school_end[$index])->format('d.m.Y') }}</p> @endif
                        @if(!empty($school_grade[$index])) <p>Abschluss: {{ $school_grade[$index] }}</p> @endif
                    </div>
                    @endforeach
                </div>
                @endif
        ...
        <div class="side-panel">
            ...
            @if(!empty($name)) <p><strong>Name</strong><br /> {{ $name }}</p><br /> @endif
            @if(!empty($birthdate)) <p><strong>Geburtsdatum</strong><br />{{ \Carbon\Carbon::parse($birthdate)->format('d.m.Y') }}</p><br /> @endif
            ...
            <div class="skill">
                @if(!empty($skill)) <p>{{ $skill }}</p> @endif
            </div>
            @endforeach
            <br />
            @endif
            ...
            <div class="skill">
                @if(!empty($language)) <p>{{ $language }} @if(!empty($language_level[$index])) - {{ $language_level[$index] }} @endif</p> @endif
            </div>
            @endforeach
            <br />
            @endif
            ...
            <div class="skill">
            @if(!empty($volunteer_task[$index])) <p>{{ $volunteer_task[$index] }}</p> @endif
                @if(!empty($volunteer)) <p>{{ $volunteer }} - {{ $volunteer_period[$index] }}</p> @endif
            </div>
            @endforeach
            @endif
        </div>
