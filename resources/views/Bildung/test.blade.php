...
<body class="MainContainer backimage">
                        ...
                        <div class="written-green-board">
                            <div class="content-written left">
                                <div class="left_scroll">
                                    <div class="group-box">
                                        <span>Dein Text:</span>
                                        <textarea name="text" id="field1" rows="10" style="width:100%;" oninput="this.style.height = '';this.style.height = this.scrollHeight + 'px'"></textarea>
                                    </div>
                                </div>
                            </div>
    ...
    $(document).ready(function () {
        var ws; // WebSocket-Verbindung

        function connectWebSocket() {
            // Ersetzen Sie 'ws://example.com/ws' mit Ihrer WebSocket-URL
            ws = new WebSocket('ws://localhost:8080');

            ws.onopen = function() {
                console.log('WebSocket-Verbindung geöffnet');
                // Senden Sie eine Nachricht an den Server, wenn die Verbindung geöffnet ist
                var formData = new FormData(document.getElementById("myForm"));
                var text = formData.get('text');
                ws.send(JSON.stringify({ toolIdentifier: 'test', text: text }));
            ...
            ws.onmessage = function(event) {
                ...
                if(data.status && data.data && data.data.response) {
                    $("#typed-text").html(data.data.response); // Für sofortige Anzeige
                    // typeText(data.data.response, "#typed-text"); // Für schrittweise Anzeige
                } else {
        ...
        $("#submitForm").click(function () {
            if(!ws || ws.readyState === WebSocket.CLOSED) {
                connectWebSocket(); // Verbindung herstellen und Daten senden
            } else {
                // WebSocket ist bereits offen, senden Sie die Daten direkt
                var formData = new FormData(document.getElementById("myForm"));
                var text = formData.get('text');
                ws.send(JSON.stringify({ toolIdentifier: 'Test', text: text }));
                $("#submitForm").text("lädt..");
        ...
        // Optional: Schrittweise Anzeige der Antwort
        function typeText(text, target) {
            var i = 0;
            var speed = 50; // Geschwindigkeit, mit der die Buchstaben erscheinen
            function typeWriter() {
                if (i < text.length) {
